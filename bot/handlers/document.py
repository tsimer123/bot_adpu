...
async def download_document(message: types.Message) -> None:
    ...
    #df2.columns[0] = (df2.columns[0]).replace(/[^\d\.\+]/g, '')
    if df2.columns[0] == 'msisdn':
        ...
        conn.close()
        await bot.send_document(message.chat.id, ('response.xlsx', fit(df)))
    elif df2.columns[0] == 'ips':
        df2['ips'] = df2['ips'].astype(str)
        df2['ips'] = df2['ips'].str.strip()
        df2 = df2.rename(columns={'ips': 'ip'})
        conn = db.connect()
        ...
        conn = db.connect()
        df2.to_sql('simki', con=conn, if_exists='replace', dtype={"ip": Text()}) 
        sql_query = pd.read_sql("SELECT ip, COALESCE(number_tel, 'Нет данных') as msisdn, COALESCE(iccid, 'Нет данных') as iccid, COALESCE(operator, 'Нет данных') as operator, COALESCE(apn, 'Нет данных') as apn, COALESCE(imei, 'Нет данных') as imei FROM simki LEFT JOIN sims USING (ip) order by simki.index;", con=conn)
        df = pd.DataFrame(sql_query, columns = ['ip', 'msisdn', 'iccid', 'operator', 'apn', 'imei'])
        conn.close()
        await bot.send_document(message.chat.id, ('response.xlsx', fit(df)))
    else:
        await message.reply(
